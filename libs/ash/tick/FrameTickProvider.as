package ash.tick
{
	...
	public class FrameTickProvider extends Signal1 implements ITickProvider
	{
		...
		public function proceed(duration:Number, tick:Number = NaN):void
		{
			if(isNaN(tick))
			{			
				tick = displayObject.stage? 1/displayObject.stage.frameRate: 31;				
			}
			
			var time:Number = tick;
			
			while(time<duration)
			{
				dispatch(tick*timeAdjustment);
				time+=tick;
			}			
		}
